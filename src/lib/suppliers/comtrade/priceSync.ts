/**
 * ComTrade price sync — /Price/items cijene, qty, globalna deaktivacija, aggregatePrices.
...
import { parseComtradeQuantity } from "./parseQuantity";
import { withPostgrestTransientRetry } from "./transient-retry";
import { resolveComtradeListPrice } from "./transform";
import type { ComtradePriceItem } from "./types";

const LOG = "[ComTrade price sync]";
...
function buildFeedMap(items: ComtradePriceItem[]): Map<string, FeedEntry> {
  const map = new Map<string, FeedEntry>();
  for (const item of items) {
    const productNo = item.productNo?.trim() || item.productID?.trim();
    if (!productNo) continue;
    const price = resolveComtradeListPrice(item);
    if (price == null) continue;
    const qty = parseComtradeQuantity(item.quantity);
    map.set(productNo, {
      price,
      deliveryDays: deliveryDaysForQty(qty),
      isActive: qty > 0
    });
  }
  return map;
...
async function fetchAllComtradeOffers(supabase: SupabaseClient): Promise<DbOfferRow[]> {
  const rows: DbOfferRow[] = [];
  let offset = 0;

  for (;;) {
    const { data, error } = await withPostgrestTransientRetry("comtradePrice.fetchOffers", async () =>
      supabase
    ...
    if (error) throw new Error(`ComTrade price sync fetch offers: ${error.message}`);
...
async function deactivateInactiveComtradeGlobally(
  ...
  for (;;) {
    const { data, error } = await withPostgrestTransientRetry("comtradePrice.activeOffers", async () =>
      supabase
        .from("supplier_products")
    ...
    if (error) throw new Error(`ComTrade price sync deactivate scan: ${error.message}`);
  ...
  for (let i = 0; i < staleIds.length; i += DEACTIVATE_CHUNK) {
    ...
    const { error: uErr } = await withPostgrestTransientRetry("comtradePrice.deactivate", async () =>
      supabase
        .from("supplier_products")
    ...
    if (uErr) throw new Error(`ComTrade price sync deactivate: ${uErr.message}`);
    deactivated += chunk.length;
  }

  if (deactivated > 0) {
    console.log(`${LOG} Deaktivirano ${deactivated} ponuda (nema u listi).`);
  }
  return deactivated;
}

export type ComtradePriceSyncResult = {
  success: boolean;
  error?: string;
  summary?: {
    feed_items: number;
...
export async function runComtradePriceSync(): Promise<ComtradePriceSyncResult> {
  ...
  try {
    ...
    const items = await client.fetchPriceItems();
    const feedMap = buildFeedMap(items);
    const fetchedIds = new Set(feedMap.keys());
    console.log(
      `${LOG} /Price/items: ${items.length} redova, ${feedMap.size} validnih (cijena + productNo)`
    );

    const dbOffers = await fetchAllComtradeOffers(supabase);
    ...
    for (const row of dbOffers) {
      ...
      if (!feed) {
        if (row.is_active) {
          const { error } = await withPostgrestTransientRetry("comtradePrice.deactRow", async () =>
            supabase
              .from("supplier_products")
              .update({ is_active: false, updated_at: updatedAt })
              .eq("supplier_id", COMTRADE_SUPPLIER_ID)
          ...
          if (error) throw new Error(`ComTrade deactivate ${id}: ${error.message}`);
          updated += 1;
          deactivatedInLoop += 1;
      ...
      const { error } = await withPostgrestTransientRetry("comtradePrice.update", async () =>
        supabase
          .from("supplier_products")
          .update({
      ...
      if (error) throw new Error(`ComTrade update ${id}: ${error.message}`);
      updated += 1;
    ...
    const deactivated = await deactivateInactiveComtradeGlobally(supabase, fetchedIds);
